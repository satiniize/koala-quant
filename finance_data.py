...
# Let x be the input time series
# 1. y = ln(x)                               # Log transform
# 2. y' = y - (β * t)                        # Remove linear trend where β is slope
# 3. y'' = (y' - μ) / σ                      # Standardize with mean μ and std dev σ
...
def get_ticker_data(ticker_symbol: str = "^JKSE", period: str = "5y"):
	feature = 'Close'
	stock_ticker = yf.Ticker(ticker_symbol)
	price_history = stock_ticker.history(period=period)
	price_history = price_history[feature].dropna()
...
def tokenize_ticker_data(data: np.ndarray, n_bins): # Maybe add randomization to fix overfitting?
