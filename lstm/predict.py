...
from model import MultiTargetFinanceModel
...
def get_initial_window(hist, ts_scaler, window_size=20):
    features = ['Open', 'High', 'Low', 'Close', 'Volume']
...
def predict_future_values(model, initial_window, tab_vector, ts_scaler, target_scaler, num_days=5):
    predictions = []
    current_window = initial_window.copy()
    device = next(model.parameters()).device

    for i in range(num_days):
        input_ts = torch.tensor(current_window, dtype=torch.float32).unsqueeze(0).to(device)
        input_tab = torch.tensor(tab_vector, dtype=torch.float32).to(device)
        model.eval()
        with torch.no_grad():
            preds_norm = model(input_ts, input_tab)  # Shape: (1, 5)
        ...
        new_day_raw = preds_original  # [Open, High, Low, Close, Volume]
        new_day_scaled = ts_scaler.transform(new_day_raw.reshape(1, -1))[0]
        current_window = np.vstack([current_window[1:], new_day_scaled])
...
if __name__ == "__main__":
    ...
    initial_window = get_initial_window(hist, ts_scaler, window_size=window_size)
    ...
    future_predictions = predict_future_values(model, initial_window, tab_vector,
                                               ts_scaler, target_scaler, num_days=num_future_days)
